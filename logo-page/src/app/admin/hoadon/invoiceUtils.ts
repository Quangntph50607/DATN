...
export const exportInvoiceToExcel = (
  ...
  const { ten, sdt } = userMap.get(userId!) || {
    ten: "N/A",
    sdt: "N/A",
  };
  ...
  saveAs(
    ...
    `HoaDon_${detail.maHD || "unknown"}.xlsx`
  );
};
...
export const exportInvoiceToDocx = (
  ...
  const userMap = getUserMap(users);
  const useId = detail.user?.id;
  const { ten, sdt } = userMap.get(useId!) || {
    ten: "N/A",
    sdt: "N/A",
  };
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({ text: "Chi tiết sản phẩm", bold: true, size: 24 }),
            ],
            spacing: { after: 200 },
          }),
          new Table({
                  ...
                  new TableRow({
                    children: [
                      ...
                      new TableCell({
                        children: [
                          new Paragraph(
                            `${(val as number)?.toLocaleString() || 0}₫`
                          ),
                        ],
                        margins: { top: 100, bottom: 100 },
                      }),
