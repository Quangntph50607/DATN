  ...
  useEffect(() => {
    ...
    if (hoaDonId) {
      ...
      try {
        ...
        const remainingCart = JSON.parse(
          localStorage.getItem("cartItems") || "[]"
        );
        setRemainingCartCount(remainingCart.length);
        console.log(
          "Số sản phẩm còn lại trong giỏ hàng:",
  ...
  const loadImages = async (products: any[]) => {
    ...
    for (const product of products) {
      try {
        ...
        if (
          product.spId?.anhSps &&
          Array.isArray(product.spId.anhSps) &&
          product.spId.anhSps.length > 0
        ) {
          // Từ backend
          const mainImage =
            typeof product.spId.anhSps[0] === "string"
        ...
        {hoaDon && isOnlinePayment(hoaDon) && (
          <div className="mb-8 p-6 rounded-xl bg-gradient-to-r from-blue-500 to-green-400 text-white shadow-lg flex flex-col items-center">
            ...
            <h2 className="text-2xl font-bold mb-2">
              Thanh toán thành công qua tài khoản!
            </h2>
            <p className="text-lg mb-2">
              Cảm ơn bạn đã sử dụng phương thức thanh toán online.
            </p>
            <p className="text-base">
              Đơn hàng của bạn đã được ghi nhận và sẽ được xử lý ngay khi hệ
              thống xác nhận thanh toán.
            </p>
            <p className="text-base mt-2">
              Bạn có thể kiểm tra trạng thái đơn hàng trong mục{" "}
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-4">
                                <div className="w-16 h-16 rounded-lg overflow-hidden border border-gray-200">
                                  {imageUrls[item.spId?.id] &&
                                  imageUrls[item.spId.id] !== null ? (
                                    <img
                                ...
                                <div>
                                  <p className="font-semibold text-gray-800 text-lg">
                                    {item.spId?.tenSanPham || "Sản phẩm"}
                                  </p>
                                  <p className="text-sm text-gray-500">
                                    SKU: {item.spId?.maSanPham || "N/A"}
                                  </p>
                                </div>
                              </div>
                            </td>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-green-100 p-8 mb-8">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Không thể tải thông tin chi tiết sản phẩm đã mua.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 p-4 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${getTrangThaiColor(
                      hoaDon.trangThai
                    ).replace("text-", "bg-")}`}
                  ></div>
                  <p
                    className={`font-semibold ${getTrangThaiColor(
                      hoaDon.trangThai
                    )}`}
                  >
                    {hoaDon.trangThai}
            ...
            <div className="space-y-6">
              <div className="bg-gray-50 p-4 rounded-xl">
                ...
                <p className="text-xl font-bold text-gray-800">
                  {hoaDon.tamTinh?.toLocaleString()}đ
                </p>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-xl">
                ...
                <p className="text-xl font-bold text-green-600">
                  -{hoaDon.soTienGiam?.toLocaleString() || 0}đ
                </p>
              </div>

              <div className="bg-gradient-to-r from-orange-500 to-red-500 p-6 rounded-xl text-white">
                <label className="block text-sm font-semibold text-orange-100 mb-2">
                  Tổng thanh toán
                </label>
                <p className="text-3xl font-bold">
                  {hoaDon.tongTien?.toLocaleString()}đ
                </p>
              </div>
            </div>
          </div>
