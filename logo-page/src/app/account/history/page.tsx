...
export default function OrderHistoryPage() {
  ...
  const { data: chiTietSanPham } = useChiTietSanPhamHoaDon(
    selectedOrder?.id || 0
  );
      ...
      const fetchDetails = async () => {
        const enrichedOrders = await Promise.all(
          orders.map(async (order: any) => {
            try {
              const chiTietData =
                await HoaDonService.getChiTietSanPhamByHoaDonId(order.id);
              const enrichedChiTiet: ChiTietSanPham[] = chiTietData.map(
                (ct: any) => {
                  const productId =
                    typeof ct.spId === "object" ? ct.spId.id : ct.spId;
                  const matched =
                    (sanPhams as unknown as SanPham[]).find(
                      (sp) => sp.id === productId
                    ) || null;
                  const giaDonHang =
                    (ct as any)?.gia ??
                    (matched as any)?.giaKhuyenMai ??
                    (matched as any)?.gia ??
                    0;
                  return {
                    ...ct,
                    sanPham: matched,
                    gia: Number(giaDonHang),
                  } as ChiTietSanPham;
                }
              );
              return {
  ...
  const filteredOrders = useMemo(() => {
    return ordersWithDetails.filter((order) => {
      const matchesStatus =
        statusFilter === "all" || order.trangThai === statusFilter;
      ...
      if (dateFilter) {
        ...
        matchesDateRange =
          ...
          orderDate.toISOString().slice(0, 10) >= dateFilter;
      }
  ...
  const parseOrderDate = (value: any): number => {
    try {
      if (!value) return 0;
      if (Array.isArray(value) && value.length >= 3) {
        const [year, month, day, hour = 0, minute = 0, second = 0, nanos = 0] =
          value;
        const millis = nanos ? Math.floor(nanos / 1000000) : 0;
        return new Date(
          year,
          (month || 1) - 1,
          day || 1,
  ...
  const sortedFilteredOrders = useMemo(() => {
    return [...filteredOrders].sort(
      (a, b) => parseOrderDate(b.ngayTao) - parseOrderDate(a.ngayTao)
    );
  }, [filteredOrders]);

  useEffect(() => setCurrentPage(1), [itemPerPage]);
  useEffect(() => setCurrentPage(1), [statusFilter, dateFilter, searchKeyword]);
  useEffect(() => setCurrentPage(1), [ordersWithDetails.length]);
  ...
  const statusOptions = [
    { value: "all", label: "Tất cả", count: ordersWithDetails.length },
    {
      value: "Đang xử lý",
      label: "Đang xử lý",
      count: ordersWithDetails.filter((o) => o.trangThai === "Đang xử lý")
        .length,
    },
    {
      value: "Đã xác nhận",
      label: "Đã xác nhận",
      count: ordersWithDetails.filter((o) => o.trangThai === "Đã xác nhận")
        .length,
    },
    {
      value: "Đang đóng gói",
      label: "Đang đóng gói",
      count: ordersWithDetails.filter((o) => o.trangThai === "Đang đóng gói")
        .length,
    },
    {
      value: "Đang vận chuyển",
      label: "Đang vận chuyển",
      count: ordersWithDetails.filter((o) => o.trangThai === "Đang vận chuyển")
        .length,
    },
    {
      value: "Đã giao",
  ...
  const totalPages = Math.ceil(sortedFilteredOrders.length / itemPerPage);
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <Button
            onClick={() => refetch()}
            className="mt-4 hover:bg-[#FFE066] text-black transition-colors duration-200"
          >
            Thử lại
          </Button>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {totalPages > 1 && (
          <OrderPagination
            ...
            totalItems={sortedFilteredOrders.length}
        ...
        <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Bạn chắc chắn muốn hủy đơn này? Hành động này không thể hoàn
                tác.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Bạn đã nhận được hàng và muốn xác nhận? Hành động này sẽ hoàn
                tất đơn hàng.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
