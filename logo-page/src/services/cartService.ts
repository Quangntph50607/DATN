...
export const cartService = {
    ...
    addToCart: async (sanPhamId: number, soLuong: number) => {
        const res = await fetchWithAuth(`${API_URL}/Create?sanPhamId=${sanPhamId}&soLuong=${soLuong}`, {
            method: "POST",
            credentials: "include",
        });
    ...
    updateCartItem: async (itemId: number, soLuong: number) => {
        const res = await fetchWithAuth(`${API_URL}/update/${itemId}?soLuong=${soLuong}`, {
            method: "PUT",
            credentials: "include",
        });
        if (!res.ok) throw new Error(await res.text());
    ...
    removeCartItem: async (itemId: number) => {
        const res = await fetchWithAuth(`${API_URL}/remove/${itemId}`, {
            method: "DELETE",
            credentials: "include",
        });
        if (!res.ok) throw new Error(await res.text());
    ...
    applyDiscount: async (phieuGiamGiaId: number) => {
        const res = await fetchWithAuth(`${API_URL}/apply-discount?phieuGiamGiaId=${phieuGiamGiaId}`, {
            method: "POST",
            credentials: "include",
        });
        if (!res.ok) throw new Error(await res.text());
